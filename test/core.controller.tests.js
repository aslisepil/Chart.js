describe('Chart.Controller', function() {
	...
	describe('controller.reset', function() {
		it('should reset the chart elements', function() {
			...
			expect(meta.data[0]._view.y).toBe(333);
			expect(meta.data[1]._view.y).toBe(183);
			expect(meta.data[2]._view.y).toBe(32);
			expect(meta.data[3]._view.y).toBe(484);

			chart.reset();

			// For a line chart, the animation state is the bottom
			expect(meta.data[0]._view.y).toBe(484);
			expect(meta.data[1]._view.y).toBe(484);
			expect(meta.data[2]._view.y).toBe(484);
			expect(meta.data[3]._view.y).toBe(484);
		});
	});
});
